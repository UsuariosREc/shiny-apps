##-----------------------------------------------------------##
#               app informe mediciones - server.R             #
##-----------------------------------------------------------##
...
shinyServer(function(input, output) {
    ...
    output$graf1 <- renderPlot({
        # Informacion cliente
        assign("nombre", input$nomb, envir=.GlobalEnv)
        assign("direc", input$direc, envir=.GlobalEnv)
        assign("repre", input$repre, envir=.GlobalEnv)
        assign("telefono", input$telf, envir= .GlobalEnv)
        assign("marca", input$marca, envir= .GlobalEnv)
        # Informacion medidor
...
#----------medidor nuevo o usado
           if(input$tipmed=="nuevo"){
               lim <- c(rep(5,2),rep(2,3))
           }else{
               lim <- c(rep(10,2),rep(4,3))   
           }
...
output$medidor <- renderText({
        ...
        if(abs(er[1])>10 | any(abs(er[2:length(er)])>4)) medidor <- "NO" 
        }
